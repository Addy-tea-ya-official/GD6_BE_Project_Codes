...
class LoadImagesAndLabels(Dataset):
    ...
    def load_pretrain_network(self, device):
        net = AODnet().to(device)
        net.load_state_dict(torch.load('D:/dataset/AOD-Net.pytorch/model/nets/AOD_19.pkl')['state_dict'])
        return net
    
    def agcwd(self,image, w=0.5):
        is_colorful = len(image.shape) >= 3
        img = self.extract_value_channel(image) if is_colorful else image
        img_pdf = self.get_pdf(img)
        ...
        enhanced_image = self.set_value_channel(image, enhanced_image) if is_colorful else enhanced_image
        return enhanced_image


    def extract_value_channel(self, color_image):
    ...
    def set_value_channel(self, color_image, value_channel):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                data_transform = torchvision.transforms.Compose([
                    torchvision.transforms.Resize((480, 704)),
                    torchvision.transforms.ToTensor()
                ])
                self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                self.network = self.load_pretrain_network(self.device)
                self.network.eval()
                haze_image_name = self.agcwd(im)
                B, G, R = cv2.split(haze_image_name)
                haze_image_name = cv2.merge((R, G, B))
                im = Image.fromarray(np.uint8(haze_image_name))
                x = data_transform(im).unsqueeze(0)
                x = x.to(self.device)
                im = self.network(x)
                torchvision.utils.save_image(im, f[-11:])
                im = cv2.imread("D:/dataset/yolov5/"+f[-11:])
                os.remove("D:/dataset/yolov5/"+f[-11:])
